...
if __name__ == '__main__':
    ...
    confusion_matrix_ts = confusion_matrix(tr_labels, np.argmax(out_train, axis=1))
    precision_ts = precision_score(tr_labels, np.argmax(out_train, axis=1), average=None)
    recall_ts = recall_score(tr_labels, np.argmax(out_train, axis=1), average=None)
    f_score_ts = f1_score(tr_labels, np.argmax(out_train, axis=1), average=None)
